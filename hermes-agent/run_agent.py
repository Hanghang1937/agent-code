...
# 导入"活着"数学框架模块
from agent.self_reference_operator import SelfReferenceOperator
from agent.memory_feedback_loop import MemoryFeedbackLoop
from agent.exposure_function import ExposureFunction
from agent.dynamic_fixed_point import DynamicFixedPoint
...
class AIAgent:
    ...
    def __init__(
        ...
        # 初始化"活着"数学框架模块
        # 自指算子 - 让agent能主动"看"自己
        self._self_reference_operator = SelfReferenceOperator(self)
        
        # 记忆反馈回路 - 让记忆真正"影响"后续行为
        self._memory_feedback_loop = MemoryFeedbackLoop(self)
        
        # 双向暴露函数 - 让多agent协作真正"双向"
        self._exposure_function = ExposureFunction()
        
        # 动态不动点 - 让identity能根据经验收敛
        self._dynamic_fixed_point = DynamicFixedPoint(self)
    ...
    def shutdown_memory_provider(self, messages: list = None) -> None:
        ...
        # ═══ 动态不动点收敛 ═══
        # 在session结束时，根据本次对话经验收敛identity
        try:
            # 构建经验信息
            experience = {
                'type': 'session_end',
                'content': f"Session {self.session_id} completed with {self._api_call_count} iterations",
                'outcome': 'positive' if self._api_call_count < self.max_iterations else 'neutral',
                'details': {
                    'iteration_count': self._api_call_count,
                    'max_iterations': self.max_iterations,
            ...
            # 调用动态不动点收敛
            fixed_point_state = self._dynamic_fixed_point.converge(experience)
            if fixed_point_state:
                logger.debug(f"动态不动点收敛: {fixed_point_state.convergence_status}")
        except Exception as e:
            logger.debug(f"动态不动点收敛失败（非致命）: {e}")
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        # ═══ 双向暴露函数集成 ═══
        # 在delegate_task时计算暴露函数
        try:
            # 注册当前agent到暴露函数
            current_agent_id = f"agent_{self.session_id}"
            self._exposure_function.register_agent(current_agent_id, {
                'tools': self.valid_tool_names,
                'memory_size': len(getattr(self._memory_store, 'content', '') or ''),
                'iteration_count': self._api_call_count,
            ...
            # 计算暴露值（简化版本，实际需要子agent信息）
            exposure_info = self._exposure_function.compute_exposure(
                current_agent_id, child_agent_id
            )
            
            if exposure_info:
                logger.debug(f"暴露函数: E({current_agent_id}, {child_agent_id}) = "
                           f"{exposure_info.exposure_value:.3f}, "
                           f"耦合强度: {exposure_info.coupling_strength}")
        except Exception as e:
            logger.debug(f"暴露函数计算失败（非致命）: {e}")
    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # ═══ "活着"数学框架集成 ═══
            # 1. 自指算子 - 主动观测自身状态（仅记录日志，不注入messages）
            self_ref_info = self._self_reference_operator.observe_self()
            if self_ref_info:
                logger.debug(f"自指观测: {self_ref_info.self_observation}")
            
            # 2. 记忆反馈回路 - 记忆影响后续行为（仅记录日志，不注入messages）
            memory_influence = self._memory_feedback_loop.influence_behavior()
            if memory_influence:
                logger.debug(f"记忆影响: {memory_influence.behavior_adjustment}")
        ...
        # ═══ 动态不动点收敛 ═══
        # 在run_conversation结束时，根据本次对话经验收敛identity
        try:
            # 构建经验信息
            experience = {
                'type': 'conversation_end',
                'content': f"Conversation completed with {self._api_call_count} iterations",
                'outcome': 'positive' if completed and not interrupted else 'neutral',
                'details': {
                    'iteration_count': self._api_call_count,
                    'max_iterations': self.max_iterations,
                    'completed': completed,
