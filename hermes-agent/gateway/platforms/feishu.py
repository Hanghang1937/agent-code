...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Track message IDs that were sent as streaming interactive cards
        # so edit_message knows to use msg_type="interactive" instead of text/post.
        self._streaming_card_ids: set = set()
        self._streaming_card_max_ids = 512
        # When True, send() uses interactive cards and edit_message updates cards.
        # Set by the gateway before creating the stream consumer.
        self.streaming_card_mode: bool = False
    ...
    async def send(
        ...
        When ``self.streaming_card_mode`` is True, the message is sent as an
        interactive card (matching OpenClaw's card streaming style).  The
        message ID is tracked so subsequent ``edit_message`` calls update the
        card rather than trying to edit a text/post message.
        """
        if not self._client:
        ...
        # Streaming card mode: send as interactive card
        if self.streaming_card_mode and len(chunks) == 1:
            card_payload = self._build_streaming_card_payload(chunks[0])
            try:
                ...
                if result.success and result.message_id:
                    self._streaming_card_ids.add(result.message_id)
                    self._prune_streaming_card_ids()
                return result
            except Exception as exc:
    ...
    async def edit_message(
        ...
        *,
        ...
        If the message was originally sent as a streaming interactive card
        (tracked in ``self._streaming_card_ids``), the update uses
        ``msg_type="interactive"`` with a card payload.  When *finalize* is
        True, the streaming indicator is removed (clean final card).
        """
        if not self._client:
            return SendResult(success=False, error="Not connected")
        ...
        # Card path: message was sent as a streaming card
        if message_id in self._streaming_card_ids:
            card_payload = self._build_streaming_card_payload(
                content, final=finalize,
            )
            try:
                body = self._build_update_message_body(
                    msg_type="interactive", content=card_payload,
                )
                ...
                if finalize:
                    self._streaming_card_ids.discard(message_id)
                return result
            except Exception as exc:
                logger.warning("[Feishu] Card edit failed, falling back to text: %s", exc)
    ...
    def _build_streaming_card_payload(self, content: str, *, final: bool = False) -> str:
        """Build a Feishu interactive card payload for streaming content.

        Matches OpenClaw's card streaming style: markdown body inside a
        wide-screen card, optionally with a streaming indicator.
        """
        # Escape markdown special chars that might break card rendering
        ...
        # Add a subtle streaming indicator while in-progress
        if not final:
            elements.append({
                "tag": "hr",
            })
    ...
    def _build_final_card_payload(self, content: str) -> str:
        """Build a clean final card (no streaming indicator)."""
        return self._build_streaming_card_payload(content, final=True)

    def _prune_streaming_card_ids(self) -> None:
        """Evict oldest streaming card IDs if the set grows too large."""
        if len(self._streaming_card_ids) > self._streaming_card_max_ids:
            excess = len(self._streaming_card_ids) - self._streaming_card_max_ids
            to_remove = list(self._streaming_card_ids)[:excess]
            for mid in to_remove:
                self._streaming_card_ids.discard(mid)
